...
class DatabaseManager:
    ...
    def create_tables(self):
        """
        Defines and creates the necessary tables for storing email analysis results.
        """
    ...
    def clear_all_data(self):
        """
        Safely deletes all records from the 'emails' and 'iocs' tables
        for a clean run, without deleting the file itself.
        """
        try:
            cursor = self.conn.cursor()
            cursor.execute("DELETE FROM emails;")
            cursor.execute("DELETE FROM sqlite_sequence WHERE name='emails';")
            cursor.execute("DELETE FROM sqlite_sequence WHERE name='iocs';")
            self.conn.commit()
    ...
    def save_analysis(self, email_data, analysis_results, score, priority):
        """
        Saves the complete analysis for a single email to the database.
        """
        cursor = self.conn.cursor()
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute('''
            INSERT INTO emails (subject, sender, risk_score, priority, analysis_timestamp)
            VALUES (?, ?, ?, ?, ?)
        ...
        for att_result in analysis_results.get("attachments", []):
            ...
            cursor.execute(
                "INSERT INTO iocs (email_id, ioc_type, ioc_value, verdict, source) VALUES (?, ?, ?, ?, ?)",
                (email_id, 'hash', att_result['sha25G'], 'Malicious' if malicious else 'Safe', 'VirusTotal')
            )
